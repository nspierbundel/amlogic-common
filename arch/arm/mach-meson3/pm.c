            ...
            if ((clks[i] == P_HHI_VID_CLK_CNTL)||(clks[i] == P_HHI_VIID_CLK_CNTL)) {
                ...
                if (aml_read_reg32(clks[i]) & (1 << 8)) {
	              		if(xtal_uart_rate_backup == 0){//if no early suspend supported
           			        struct clk* sys_clk = clk_get_sys("clk_xtal", NULL);
    ...
    if (flag) {
        for (i = EARLY_PLL_COUNT - 1; i >= 0; i--) {
            if (early_pll_flag[i]) {
                if ((early_plls[i]==P_HHI_VID_PLL_CNTL)||(early_plls[i]==P_HHI_VIID_PLL_CNTL)){
                  	aml_clr_reg32_mask(early_plls[i],(1<<30));
                    early_pll_flag[i] = 0;
                }
                else{
                   aml_clr_reg32_mask(early_plls[i],(1<<15));
                   early_pll_flag[i] = 0;
                }
        ...
        for (i = 0; i < EARLY_PLL_COUNT; i++) {
            if ((early_plls[i]==P_HHI_VID_PLL_CNTL)||(early_plls[i]==P_HHI_VIID_PLL_CNTL))
            	...
            	early_pll_flag[i] = aml_get_reg32_bits(early_plls[i],15,1) ? 0 : 1;
            if (early_pll_flag[i]) {
                printk(KERN_INFO "early pll %s(%x) off\n", early_plls_name[i], early_plls[i]);
                if ((early_plls[i]==P_HHI_VID_PLL_CNTL)||(early_plls[i]==P_HHI_VIID_PLL_CNTL))
                   aml_set_reg32_mask(early_plls[i], (1 << 30));
                else
                   aml_set_reg32_mask(early_plls[i], (1 << 15));
            }
        }
    }
}
...
static void meson_pm_suspend(void)
{
    ...
    auto_clk_gating_setup(2,                             // select 100uS timebase
                          ...
                          1);                         // 1uS enable delay
#endif
		aml_clr_reg32_mask(P_HHI_A9_CLK_CNTL, 1<<7);
		aml_set_reg32_mask(P_HHI_SYS_PLL_CNTL, 1 << 15);
#ifdef ADJUST_CORE_VOLTAGE
    ...
    meson_power_suspend();
#else
    aml_set_reg32_mask(P_A9_0_IRQ_IN2_INTR_MASK, pdata->power_key); //enable rtc interrupt only
    ...
    udelay(100);
#endif
		aml_clr_reg32_mask(P_HHI_SYS_PLL_CNTL, (1 << 15)); //turn on sys pll
    ...
    if (pdata->set_vccx2) {
		...
		aml_set_reg32_mask(P_HHI_A9_CLK_CNTL , (1 << 7)); //a9 use pll
...
static int meson_pm_prepare(void)
{
    ...
    mask_save[0] = aml_read_reg32(P_A9_0_IRQ_IN0_INTR_MASK);
    mask_save[1] = aml_read_reg32(P_A9_0_IRQ_IN1_INTR_MASK);
    mask_save[2] = aml_read_reg32(P_A9_0_IRQ_IN2_INTR_MASK);
    mask_save[3] = aml_read_reg32(P_A9_0_IRQ_IN3_INTR_MASK);
    aml_write_reg32(P_A9_0_IRQ_IN0_INTR_MASK, 0x0);
    aml_write_reg32(P_A9_0_IRQ_IN1_INTR_MASK, 0x0);
    aml_write_reg32(P_A9_0_IRQ_IN2_INTR_MASK, 0x0);
    aml_write_reg32(P_A9_0_IRQ_IN3_INTR_MASK, 0x0);
#ifndef CONFIG_MESON_SUSPEND
    meson_sram_push(meson_sram_suspend, meson_cpu_suspend,
                    meson_cpu_suspend_sz);
#endif
...
static void meson_pm_finish(void)
{
    ...
    aml_write_reg32(P_A9_0_IRQ_IN0_INTR_MASK, mask_save[0]);
    aml_write_reg32(P_A9_0_IRQ_IN1_INTR_MASK, mask_save[1]);
    aml_write_reg32(P_A9_0_IRQ_IN2_INTR_MASK, mask_save[2]);
    aml_write_reg32(P_A9_0_IRQ_IN3_INTR_MASK, mask_save[3]);
#ifdef CONFIG_MESON_SUSPEND
